...
def get_markers_mixes():
    ...
    for i in range(K):
        markers_cell_types[i] = markers_cell_types[i][markers_cell_types[i].isin(usable_markers)].head(N_COL)
    ...
    assert len(markers_all)==K*N_COL

    return markers_cell_types, markers_all, mixes

def preprocess_mixes(markers, mixes, true_proportions, split=False):
    ...
    x_train = pd.concat([scenarios_x[2],scenarios_x[0],scenarios_x[3]], axis=0)
    x_test = scenarios_x[1]
    prop_train = pd.concat([scenarios_y[2],scenarios_y[0],scenarios_y[3]], axis=0)
